...
TEST(Arithmetic, divide) {
    const uint_t big_val  (0xfedbca9876543210ULL);
    const uint_t small_val(0xffffULL);
    const uint_t res_val  (0xfedcc9753fc9ULL);
...
TEST(External, divide) {
    ...
    const uint_t val(0x7bULL);

    EXPECT_EQ(t   /  val, false);
    EXPECT_EQ(f   /  val, false);
    EXPECT_EQ(u8  /  val, uint_t(0x1ULL));
    EXPECT_EQ(u16 /  val, uint_t(0x163ULL));
    EXPECT_EQ(u32 /  val, uint_t(0x163356bULL));
    EXPECT_EQ(u64 /  val, uint_t(0x163356b88ac0de0ULL));
