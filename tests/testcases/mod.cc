...
TEST(Arithmetic, modulo) {
    // has remainder
    const uint256_t val    (0xffffffffffffffffULL, 0xffffffffffffffffULL);
    const uint256_t val_mod(0xfedcba9876543210ULL);

    EXPECT_EQ(val % val_mod, uint256_t(0x7f598f328cc265bfULL));

    // no remainder
    const uint256_t val_0  (0xfedcba9876543210, 0);
